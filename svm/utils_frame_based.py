# utility functions: frame-model
# based on "code_written/4.construct_df/read_video_trin.py"
...
def display_stats(closeness_list,
                  blinks_list,
                  video_info=None,
                  skip_n=0,
                  test=False):
...
# process a given video file (no-display version)
...
# the computed eye aspect ratio for the frame
            ...
            cv2.putText(frame, "SOP: {:.2f}".format(sop), (10, 180),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.putText(frame, "TIME: {:.1f}".format(current_frame / fps),
                        (10, 280), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255),
                        2)
    ...
    frame_info_df = pd.DataFrame(
        frame_info_list)  # build a dataframe from frame_info_list
